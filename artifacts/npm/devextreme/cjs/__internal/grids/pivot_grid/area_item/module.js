...
var AreaItem = class_1.default.inherit({
    ...
    _getMainElementMarkup: function () {
        return dom_adapter_1.default.createElement('tbody');
    },
    ...
    _renderTableContent: function (tableElement, data) {
        ...
        for (i = 0; i < rowsCount; i += 1) {
            row = data[i];
            rowClassNames = [];
            var tr = dom_adapter_1.default.createElement('tr');
            for (j = 0; j < row.length; j += 1) {
                cell = row[j];
                this._getRowClassNames(i, cell, rowClassNames);
                var td = dom_adapter_1.default.createElement('td');
                if (cell) {
                    ...
                    if (styleOptions.cssArray.length) {
                        style_1.setStyle(td, styleOptions.cssArray.join(';'));
                    }
                    if (styleOptions.classArray.length) {
                        td.setAttribute('class', styleOptions.classArray.join(' '));
                    }
                    if (type_1.isDefined(cell.expanded)) {
                        var div = dom_adapter_1.default.createElement('div');
                        div.classList.add('dx-expand-icon-container');
                        var span_1 = dom_adapter_1.default.createElement('span');
                        span_1.classList.add(PIVOTGRID_EXPAND_CLASS);
                ...
                var span = dom_adapter_1.default.createElement('span');
                ...
                td.appendChild(span);
                if (cell.sorted) {
                    var span_2 = dom_adapter_1.default.createElement('span');
                    span_2.classList.add('dx-icon-sorted');
    ...
    setColumnsWidth: function (values) {
        ...
        for (i = 0; i < columnsCount; i += 1) {
            var col = dom_adapter_1.default.createElement('col');
    ...
    _renderVirtualContent: function () {
        ...
        if (!that._virtualContent && that.option('scrolling.mode') === 'virtual') {
            that._virtualContent = renderer_1.default('<div>').addClass('dx-virtual-content').insertBefore(that._tableElement);
        }
    },
    reset: function () {
        var that = this;
    ...
    _updateFakeTableVisibility: function () {
        ...
        if (tableElement.style.top === fakeTableElement.style.top
    ...
    render: function (rootElement, data) {
        ...
        that._colgroupElement = renderer_1.default('<colgroup>').appendTo(that._tableElement);
    ...
    _getMemoizeScrollTo: function () {
        var _this = this;
        var _a;
        this._memoizeScrollTo = (_a = this._memoizeScrollTo) !== null && _a !== void 0 ? _a : getMemoizeScrollTo_1.getMemoizeScrollTo(function () { return _this._getScrollable(); });
    ...
    scrollTo: function (pos, force) {
        ...
        var scrollablePos = __assign(__assign({}, pos), { left: rtlEnabled && (areaName === 'column' || areaName === 'data')
...
exports.default = { AreaItem: AreaItem, getRealElementWidth: getRealElementWidth };
