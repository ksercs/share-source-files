...
export default {
    ...
    getWrappedLookupDataSource(column, dataSource, filter) {
        ...
        const loadUniqueRelevantItems = (loadOptions) => {
            ...
            const canUseCache = cachedUniqueRelevantItems && (!hasGroupPaging
        ...
        const lookupDataSource = Object.assign(Object.assign({}, lookupDataSourceOptions), { __dataGridSourceFilter: filter, load: (loadOptions) => {
                // @ts-expect-error
                const d = new Deferred();
                loadUniqueRelevantItems(loadOptions).done((items) => {
                    ...
                    const filter = this.combineFilters(items.flatMap((data) => data.key).map((key) => [
                        column.lookup.valueExpr, key,
                    ]), 'or');
                    const newDataSource = new DataSource(Object.assign(Object.assign(Object.assign({}, lookupDataSourceOptions), loadOptions), { filter: this.combineFilters([filter, loadOptions.filter], 'and'), paginate: false }));
                    newDataSource
                        // @ts-expect-error
                        .load()
                        .done(d.resolve)
                ...
                return d;
            }, key: column.lookup.valueExpr, byKey(key) {
                const d = Deferred();
                this.load({
