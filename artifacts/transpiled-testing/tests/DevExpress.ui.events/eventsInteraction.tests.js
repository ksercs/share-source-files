...
(["testing/tests/DevExpress.ui.events/eventsInteraction.tests.js"], ["jquery","core/utils/common","core/utils/dom","core/devices","events/utils/index","events/core/emitter","events/gesture/emitter.gesture","events/core/emitter_registrator","events/core/emitter.feedback","events/gesture/emitter.gesture.scroll","events/hold","events/swipe","events/transform","events/drag","events/dblclick","../../helpers/pointerMock.js"], true, function($__System) {
var require = this.require, exports = this.exports, module = this.module;
$__System.registerDynamic('testing/tests/DevExpress.ui.events/eventsInteraction.tests.js', ['jquery', 'core/utils/common', 'core/utils/dom', 'core/devices', 'events/utils/index', 'events/core/emitter', 'events/gesture/emitter.gesture', 'events/core/emitter_registrator', 'events/core/emitter.feedback', 'events/gesture/emitter.gesture.scroll', 'events/hold', 'events/swipe', 'events/transform', 'events/drag', 'events/dblclick', '../../helpers/pointerMock.js'], true, function ($__require, exports, module) {
    var global = this || self,
        GLOBAL = global;
    const $ = $__require('jquery');
    const noop = $__require('core/utils/common').noop;
    const domUtils = $__require('core/utils/dom');
    const devices = $__require('core/devices');
    const eventUtils = $__require('events/utils/index');
    const Emitter = $__require('events/core/emitter');
    const GestureEmitter = $__require('events/gesture/emitter.gesture');
    const registerEmitter = $__require('events/core/emitter_registrator');
    ...
    const holdEvent = $__require('events/hold');
    const swipeEvents = $__require('events/swipe');
    const transformEvent = $__require('events/transform');
    const dragEvents = $__require('events/drag');
    const dblclickEvent = $__require('events/dblclick');
    ...
    QUnit.test('emitter should be removed only after last emitter event unsubscribed', function (assert) {
        const $element = $('#element').on('dxteststart', noop).on('dxtestend', noop);
    ...
    QUnit.test('text selection should be reset on gesture start', function (assert) {
        ...
        try {
            domUtils.clearSelection = function () {
                assert.ok(true, 'selection cleared');
            };
    ...
    QUnit.test('text selection shouldn\'t be reset on native scroll', function (assert) {
        ...
        try {
            domUtils.clearSelection = function () {
                assert.ok(false, 'selection cleared');
            };
    ...
    QUnit.test('test selection should not reset on mouseWheel and touch events', function (assert) {
        ...
        try {
            ...
            $element.on('dxscrollstart', { validate: function () {
                    return true;
                } }, noop);
    ...
    const testContinuous = function (config) {
        QUnit.test(config.element.direction + ' ' + config.element.event, function (assert) {
            ...
            pointerMock($element).start().down().move(config.startDirection === 'horizontal' ? 10 : 0, config.startDirection === 'vertical' ? 10 : 0).move(config.endDirection === 'horizontal' ? 10 : 0, config.endDirection === 'vertical' ? 10 : 0);
    ...
    const testNestedGesture = function (config) {
        QUnit.test(config.parent.direction + ' ' + config.parent.event.replace('dx', '') + ' in ' + config.child.direction + ' ' + config.child.event.replace('dx', ''), function (assert) {
            ...
            pointerMock($child).start().down().move(config.direction === 'horizontal' ? 10 : 0, config.direction === 'vertical' ? 10 : 0);
    ...
    const testNestedGestureAcceptingOnUnsubscribing = function (parentGesture, childGesture) {
        QUnit.test(parentGesture.replace('dx', '') + ' in ' + childGesture.replace('dx', ''), function (assert) {
            ...
            pointerMock($child).start().down().move(0, 10);
    ...
    QUnit.test('inactive should be fired after swipe without timeout if swipe started with delay', function (assert) {
        ...
        $parent.on(feedbackEvents.active, { timeout: 10 }, noop).on(feedbackEvents.inactive, { timeout: 100 }, function () {
            inactiveFired++;
        });
    ...
    QUnit.test('active emitter should not be reset if inactive emitter was unsubscribed', function (assert) {
        ...
        const $element = $('#element').on(dragEvents.move, noop).on(dragEvents.end, function () {
        ...
        $element.on(scrollEvents.move, noop).off(scrollEvents.move);
    ...
    QUnit.module('gesture cover', {
        ...
        if (devices.real().deviceType === 'desktop') {
            ...
            $.each([['hover', 'pointer-events', 'all', true], ['cursor', 'cursor', 'move', false]], function (_, config) {
                const name = config[0];
                const prop = config[1];
...
(function(factory) {
  if (typeof define == 'function' && define.amd)
    define(["jquery","core/utils/common","core/utils/dom","core/devices","events/utils","events/core/emitter","events/gesture/emitter.gesture","events/core/emitter_registrator","events/core/emitter.feedback","events/gesture/emitter.gesture.scroll","events/hold","events/swipe","events/transform","events/drag","events/dblclick","../../helpers/pointerMock.js"], factory);
  else if (typeof module == 'object' && module.exports && typeof require == 'function')
    module.exports = factory(require("jquery"), require("core/utils/common"), require("core/utils/dom"), require("core/devices"), require("events/utils"), require("events/core/emitter"), require("events/gesture/emitter.gesture"), require("events/core/emitter_registrator"), require("events/core/emitter.feedback"), require("events/gesture/emitter.gesture.scroll"), require("events/hold"), require("events/swipe"), require("events/transform"), require("events/drag"), require("events/dblclick"), require("../../helpers/pointerMock.js"));
  else
    throw new Error("Module must be loaded as AMD or CommonJS");
});
//# sourceMappingURL=eventsInteraction.tests.js.map