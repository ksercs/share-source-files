System.register(["jquery", "core/config", "core/devices", "ui/date_range_box", "ui/date_box", "core/utils/type", "core/utils/type.js", "animation/fx", "events/hover", "../../helpers/keyboardMock.js", "ui/popup/ui.popup", "ui/validator", "generic_light.css!"], function (_export, _context) {
  "use strict";

  var $, config, devices, DateRangeBox, DateBox, isRenderer, isObject, fx, hoverEvents, keyboardMock, Popup, WIDGET_CLASS, BUTTON_CLASS, DROP_DOWN_EDITOR_BUTTON_CLASS, DROP_DOWN_EDITOR_BUTTONS_CONTAINER_CLASS, TEXTEDITOR_INPUT_CLASS, POPUP_CONTENT_CLASS, CLEAR_BUTTON, STATE_FOCUSED_CLASS, STATE_HOVER_CLASS, VALIDATION_MESSAGE_CLASS, SHOW_INVALID_BADGE_CLASS, INVALID_CLASS, CALENDAR_CELL_CLASS, APPLY_BUTTON_SELECTOR, getStartDateBoxInstance, getEndDateBoxInstance, getButtonsContainers, getButtons, getClearButton, moduleConfig;
  return {
    ...
    }, function (_uiDate_range_box) {
      DateRangeBox = _uiDate_range_box.default;
    }, function (_uiDate_box) {
      DateBox = _uiDate_box.default;
    }, function (_coreUtilsType) {
      isRenderer = _coreUtilsType.isRenderer;
    }, function (_coreUtilsTypeJs) {
      isObject = _coreUtilsTypeJs.isObject;
    }, function (_animationFx) {
      fx = _animationFx.default;
    }, function (_eventsHover) {
      hoverEvents = _eventsHover.default;
    }, function (_helpersKeyboardMockJs) {
      keyboardMock = _helpersKeyboardMockJs.default;
    }, function (_uiPopupUiPopup) {
      Popup = _uiPopupUiPopup.default;
    }, function (_uiValidator) {}, function (_generic_lightCss) {}],
    execute: function () {
      QUnit.testStart(() => {
        const markup = '<div id="dateRangeBox"></div><div id="dateRangeBox2"></div>';
      ...
      VALIDATION_MESSAGE_CLASS = 'dx-invalid-message';
      SHOW_INVALID_BADGE_CLASS = 'dx-show-invalid-badge';
      INVALID_CLASS = 'dx-invalid';
      CALENDAR_CELL_CLASS = 'dx-calendar-cell';
      APPLY_BUTTON_SELECTOR = '.dx-popup-done.dx-button';
      getStartDateBoxInstance = dateRangeBoxInstance => dateRangeBoxInstance.getStartDateBox();
      getEndDateBoxInstance = dateRangeBoxInstance => dateRangeBoxInstance.getEndDateBox();
      getButtonsContainers = $element => $element.find(`> .${DROP_DOWN_EDITOR_BUTTONS_CONTAINER_CLASS}`);
      getButtons = $element => $element.find(`.${DROP_DOWN_EDITOR_BUTTON_CLASS}`);
      getClearButton = $element => getButtonsContainers($element).find(`.${CLEAR_BUTTON}`);
      moduleConfig = {
        beforeEach: function () {
          fx.off = true;
          const init = options => {
      ...
      QUnit.module('Behavior', moduleConfig, () => {
        ...
        ['startDateBox', 'endDateBox'].forEach(dateBoxName => {
          QUnit.test(`${dateBoxName} should update value on DateRangeBox value change`, function (assert) {
            ...
            const dateBox = dateBoxName === 'startDateBox' ? getStartDateBoxInstance(this.instance) : getEndDateBoxInstance(this.instance);
          ...
          QUnit.test(`DateRangeBox should not update value on ${dateBoxName} value change if the value is the same`, function (assert) {
            ...
            const onValueChangedHandler = sinon.stub();
            const dateBox = isStartDateBox ? getStartDateBoxInstance(this.instance) : getEndDateBoxInstance(this.instance);
          ...
          beforeEach: function () {
            ...
            this.testValue = (assert, value, callCount) => {
              ...
              if (callCount) {
                const {
                  value: eventValue
                } = this.onValueChangedHandler.getCall(0).args[0];
      ...
      QUnit.module('Option synchronization', moduleConfig, () => {
        ...
        QUnit.test('DateRangeBox "startDateInputAttr", "endDateInputAttr" option values should be passed in "inputAttr" option of startDateBox and endDateBox after change in runtime respectively', function (assert) {
          ...
          assert.deepEqual(this.instance.getStartDateBox().option('inputAttr'), {
            id: 'startDateInput'
          }, 'startDateBox inputAttr option has correct value');
          assert.deepEqual(this.instance.getEndDateBox().option('inputAttr'), {
            id: 'endDateInput'
          }, 'endDateBox inputAttr option has correct value');
        });
        ...
        QUnit.test('CalendarOptions should be passed to startDateBox on runtime change', function (assert) {
          const startDateBox = getStartDateBoxInstance(this.instance);
          this.instance.option('calendarOptions', {
            showWeekNumbers: true
          });
          assert.strictEqual(startDateBox.option('calendarOptions.showWeekNumbers'), true);
        });
        ...
        QUnit.test('DropDownOptions should be passed to startDateBox on runtime change', function (assert) {
          const startDateBox = getStartDateBoxInstance(this.instance);
          this.instance.option('dropDownOptions', {
            width: 800
          });
          assert.strictEqual(startDateBox.option('dropDownOptions.width'), 800);
        });
        ...
        ['startDateBox', 'endDateBox'].forEach(dateBoxName => {
          ...
          QUnit.test('DateRangeBox startDateText and endDateText options should return text option of dateboxes correctly after change value in runtime', function (assert) {
            this.reinit({});
            this.instance.option('value', ['2021/09/17', '2021/09/24']), assert.deepEqual(new Date(this.instance.option('startDateText')), new Date('2021/09/17'));
            assert.deepEqual(new Date(this.instance.option('endDateText')), new Date('2021/09/24'));
            assert.strictEqual(this.instance.option('startDateText'), this.instance.getStartDateBox().option('text'));
            assert.strictEqual(this.instance.option('endDateText'), this.instance.getEndDateBox().option('text'));
          });
      ...
      QUnit.module('Validation', {
        ...
        failInternalValidation(keyboard) {
          keyboard.press('backspace').type('f').change();
        },
        successInternalValidation(keyboard) {
          keyboard.press('backspace').change();
        },
        raiseExternalValidation(keyboard) {
          keyboard.caret({
            start: 0,
            end: 10
          }).type('5/5/2023').change();
        }
      }, () => {
              ...
              this.instance.option({
                onOptionChanged: ({
                  ...
                  if (name === 'validationErrors') {
                    ...
                    }], 'validationErrors previousValue');
        ...
        QUnit.module('validation icon', () => {
          ...
          QUnit.test('end dateBox validation icon should be shown if external validation is failed after end dateBox second value change', function (assert) {
            ...
            }).type('5/5/2023').change().caret({
              start: 0,
              end: 10
            }).type('4/5/2023').change();
            assert.strictEqual(endDateBox.$element().hasClass(SHOW_INVALID_BADGE_CLASS), true, `${SHOW_INVALID_BADGE_CLASS} class is added`);
            assert.strictEqual(endDateBox.$element().hasClass(INVALID_CLASS), true, `${INVALID_CLASS} class is added`);
          });
        });
        QUnit.module('internal validation', () => {
          ...
          QUnit.test('external validation raise on value change should not restore internal validation errors', function (assert) {
            ...
            assert.deepEqual(validationErrors[1], {
              message: 'Start value must be a date'
            }, 'internal error is not restored');
          });
          ...
          ['startDateBox', 'endDateBox'].forEach(dateBoxName => {
            QUnit.module(dateBoxName, {
              beforeEach: function () {
                this.dateBox = dateBoxName === 'startDateBox' ? getStartDateBoxInstance(this.instance) : getEndDateBoxInstance(this.instance);
              ...
              QUnit.test('internal validation success should not set dateRangeBox isValid to true if there is an other error', function (assert) {
                this.instance.option('isValid', false);
                this.instance.option('validationError', {
                  message: 'external error'
                });
                const keyboard = keyboardMock(this.$dateBoxInput);
                this.failInternalValidation(keyboard);
