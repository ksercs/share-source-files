...
(["testing/tests/DevExpress.ui.widgets/buttonGroup.tests.js"], ["core/utils/size","jquery","ui/button","ui/button_group","core/devices","events/core/events_engine","../../helpers/keyboardMock.js","../../helpers/pointerMock.js","../../helpers/registerKeyHandlerTestHelper.js","generic_light.css!"], false, function($__System) {
var require = this.require, exports = this.exports, module = this.module;
$__System.register("testing/tests/DevExpress.ui.widgets/buttonGroup.tests.js", ["core/utils/size", "jquery", "ui/button", "ui/button_group", "core/devices", "events/core/events_engine", "../../helpers/keyboardMock.js", "../../helpers/pointerMock.js", "../../helpers/registerKeyHandlerTestHelper.js", "generic_light.css!"], function($__export) {
  "use strict";
  var getWidth,
      getHeight,
      getOuterHeight,
    ...
    execute: function() {
      ...
      QUnit.module('options', function() {
        QUnit.test('items=[item1.elementAttr = { attr1="test1", attr2="test2", attr3="test3" }]', function(assert) {
          var buttonGroup = $('#buttonGroup').dxButtonGroup({
            keyExpr: 'id',
            items: [{
              id: 1,
              text: 'button 1',
          ...
          var $button = buttonGroup.find('.dx-button');
      ...
      QUnit.module('option changed', {
        createButtonGroup: function(options) {
          options = options || {items: [{text: 'button 1'}, {text: 'button 2'}]};
        ...
        QUnit.test('change items', function(assert) {
          this.buttonGroup.option('items', [{text: 'left'}, {text: 'right'}]);
          var buttons = $(("." + BUTTON_CLASS)).map(function(_, $button) {
            return $($button).dxButton('instance');
          });
        ...
        QUnit.test('change the width option', function(assert) {
          ...
          var buttonsSelector = ("." + BUTTON_CLASS);
        ...
        QUnit.test('buttonGroup height option', function(assert) {
          var $buttonGroup = this.createButtonGroup({height: 500}).$element();
        ...
        QUnit.test('change the width option when item has template', function(assert) {
          ...
          var $items = $(("." + BUTTON_GROUP_ITEM_CLASS));
        ...
        QUnit.test('template property of the item should be passed to the inner dxButton', function(assert) {
          var buttonGroup = this.createButtonGroup({items: [{
              text: 'button 1',
              template: function() {
                return 'Template';
              }
            }]});
          var $buttonGroup = buttonGroup.$element();
        ...
        QUnit.test('custom item property should be passed to buttonTemplate function', function(assert) {
          var templateMock = sinon.stub().returns('Template');
          var item1 = {
            text: 'button 1',
            icon: 'box',
            custom: 1
        ...
        QUnit.test('custom item property should not be passed to default buttonTemplate', function(assert) {
          ...
          this.createButtonGroup({items: [item1, item2]});
          var buttons = $(("." + BUTTON_CLASS)).map(function(_, $button) {
            return $($button).dxButton('instance');
          });
        ...
        QUnit.test('button has aria-pressed=false attribute if unselected', function(assert) {
          ...
          var $firstButton = $(("." + BUTTON_CLASS)).eq(0);
          var $secondButton = $(("." + BUTTON_CLASS)).eq(1);
        ...
        QUnit.test('it should be possible to set full set of options for each button', function(assert) {
          var $element = $('#widget').dxButtonGroup({items: [{
              text: 'button 1',
              width: 24,
              elementAttr: {class: 'test'},
              customOption: 'Test option'
            }]});
          var buttonsSelector = ("." + BUTTON_CLASS);
          var button = $element.find(buttonsSelector).eq(0).dxButton('instance');
        ...
        QUnit.test('onClick can be redefined', function(assert) {
          ...
          var buttonsSelector = ("." + BUTTON_CLASS);
          var button = $element.find(buttonsSelector).eq(0);
          eventsEngine.trigger(button, 'dxclick');
      ...
      QUnit.module('Events', function() {
        var ButtonGroupEventsTestHelper = function() {
          function ButtonGroupEventsTestHelper(eventName, isItemClickInInitialOption, isDisabled, isItemDisabled) {
          ...
          return ($traceurRuntime.createClass)(ButtonGroupEventsTestHelper, {
            ...
            performAction: function() {
              if (this.eventName === 'click') {
                ...
                pointerMock(this._getButtonGroupItem(0)).start('touch').down().up();
              } else {
                if (!this.isDisabled)
                  keyboardMock(this.buttonGroup.$element()).press(this.eventName);
              }
            },
            checkAsserts: function(assert) {
              if ((this.isDisabled || this.isItemDisabled)) {
                ...
                var e = this.handler.getCall(0).args[0];
        ...
        [true, false].forEach(function(isDisabled) {
          [true, false].forEach(function(isItemDisabled) {
            [true, false].forEach(function(isItemClickInInitialOption) {
              ['click', 'touch', 'space', 'enter'].forEach(function(eventName) {
                var config = (" " + eventName + ", onItemClick is initial option=" + isItemClickInInitialOption + ", disabled: " + isDisabled + " " + (isItemDisabled ? (", item.disabled=" + isItemDisabled) : ''));
                QUnit.test('Check onItemClick for' + config, function(assert) {
                  var helper = new ButtonGroupEventsTestHelper(eventName, isItemClickInInitialOption, isDisabled, isItemDisabled);
      ...
      if (devices.current().deviceType === 'desktop') {
        registerKeyHandlerTestHelper.runTests({
          createWidget: function($element, options) {
            return $element.dxButtonGroup($.extend({items: [{text: 'text'}]}, options)).dxButtonGroup('instance');
          },
...
(function(factory) {
  if (typeof define == 'function' && define.amd)
    define(["core/utils/size","jquery","ui/button","ui/button_group","core/devices","events/core/events_engine","../../helpers/keyboardMock.js","../../helpers/pointerMock.js","../../helpers/registerKeyHandlerTestHelper.js","generic_light.css!"], factory);
  else if (typeof module == 'object' && module.exports && typeof require == 'function')
    module.exports = factory(require("core/utils/size"), require("jquery"), require("ui/button"), require("ui/button_group"), require("core/devices"), require("events/core/events_engine"), require("../../helpers/keyboardMock.js"), require("../../helpers/pointerMock.js"), require("../../helpers/registerKeyHandlerTestHelper.js"), require("generic_light.css!"));
  else
    throw new Error("Module must be loaded as AMD or CommonJS");
});
//# sourceMappingURL=buttonGroup.tests.js.map