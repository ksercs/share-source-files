...
(["testing/tests/DevExpress.ui.widgets/listParts/editingTests.js"], ["jquery","core/utils/type","../../../helpers/executeAsyncMock.js","../../../helpers/keyboardMock.js","data/data_source/data_source","data/array_store","ui/list"], false, function($__System) {
var require = this.require, exports = this.exports, module = this.module;
$__System.register("testing/tests/DevExpress.ui.widgets/listParts/editingTests.js", ["jquery", "core/utils/type", "../../../helpers/executeAsyncMock.js", "../../../helpers/keyboardMock.js", "data/data_source/data_source", "data/array_store", "ui/list"], function($__export) {
  "use strict";
  var $,
      typeUtils,
      executeAsyncMock,
  ...
  return {
    ...
    execute: function() {
      ...
      QUnit.test('index should be correct for plain list', function(assert) {
        var items = [{a: 0}, {a: 1}];
        var $list = $('#templated-list').dxList({items: items});
      ...
      QUnit.test('it should be possible to select an item in the grouped list by primitive index', function(assert) {
        ...
        assert.deepEqual(list.option('selectedItem'), {
          key: 'second',
          items: [{a: 2}]
        }, 'selectedItem is correct');
        assert.deepEqual(list.option('selectedItems'), [{
          key: 'second',
          items: [{a: 2}]
        }], 'selectedItems is correct');
      ...
      QUnit.test('it should be possible to select an item with index bigger then 255 in the grouped list (T996851)', function(assert) {
        ...
        var list = $('#list').dxList({
          dataSource: [{
            key: 'someKey',
            items: $traceurRuntime.spread(Array(300).keys())
          }],
          grouped: true,
          selectionMode: 'single',
      ...
      QUnit.test('items reordering by keyboard', function(assert) {
        ...
        var $list = $('#list').dxList({
          items: items,
          editEnabled: true,
          itemDragging: {allowReordering: false},
          focusStateEnabled: true
        });
        var list = $list.dxList('instance');
        var keyboard = keyboardMock($list.find('[tabindex=0]'));
        var $lastItem = $list.find('.' + LIST_ITEM_CLASS).eq(2);
      ...
      QUnit.test('unselectAll method should not unselect disabled items  (T1050340)', function(assert) {
        ...
        instance.unselectAll();
        assert.deepEqual(instance.option('selectedItems'), items.filter(function(item) {
          return item.disabled;
        }), 'disabled items are not unselected');
      });
      ...
      QUnit.test('selectedItems should not be cleaned after reordering if store key specified', function(assert) {
        ...
        var item1 = $('#list').find(toSelector(LIST_ITEM_CLASS)).eq(1).get(0);
      ...
      QUnit.test('reorderItem method should return a Promise', function(assert) {
        var listInstance = $('#list').dxList({dataSource: [1, 2, 3]}).dxList('instance');
        listInstance.selectItem(0);
        var promise = listInstance.reorderItem(0, 1);
        var $items = $('#list').find(toSelector(LIST_ITEM_CLASS));
        var firstItemText = $items.eq(0).text();
        var secondItemText = $items.eq(1).text();
      ...
      QUnit.module('selecting in grouped list', {beforeEach: function() {
          this.data = [{
            key: 'first',
            items: [{a: 0}, {a: 1}, {a: 2}]
          }, {
      ...
      QUnit.test('item should be selectable by click on it in the grouped list', function(assert) {
        ...
        var $items = $(list.itemElements());
        $items.eq(0).trigger('dxclick');
        assert.ok($items.eq(0).hasClass('dx-list-item-selected'), 'item became selected');
      });
      QUnit.module('selecting in grouped list with single mode', {beforeEach: function() {
          this.data = [{
            key: 'first',
            items: [{a: 0}, {a: 1}, {a: 2}]
          }, {
      ...
      QUnit.module('reordering in grouped items', {beforeEach: function() {
          this.data = [{
            key: 'first',
            items: [{a: 0}, {a: 1}, {a: 2}]
          }, {
      ...
      QUnit.test('reorderItem should swap items by index', function(assert) {
        ...
        var list = $list.dxList('instance');
        var refreshItemsSpy = sinon.spy(list, '_refreshItemElements');
        var groups = function() {
          return $list.find(toSelector(LIST_GROUP_CLASS));
        };
        var item = function(group, item) {
      ...
      QUnit.test('onItemReordered should be fired if items reordered', function(assert) {
        var $list = $('#templated-list').dxList({
          ...
          onItemReordered: function(args) {
            assert.deepEqual(args.fromIndex, {
              group: 0,
              item: 1
            }, 'correct from index');
            assert.deepEqual(args.toIndex, {
              group: 1,
              item: 1
            }, 'correct to index');
          }
        ...
        var list = $list.dxList('instance');
        var item = function(group, item) {
          return $list.find(toSelector(LIST_GROUP_CLASS)).eq(group).find(toSelector(LIST_ITEM_CLASS)).eq(item).get(0);
        };
...
(function(factory) {
  if (typeof define == 'function' && define.amd)
    define(["jquery","core/utils/type","../../../helpers/executeAsyncMock.js","../../../helpers/keyboardMock.js","data/data_source/data_source","data/array_store","ui/list"], factory);
  else if (typeof module == 'object' && module.exports && typeof require == 'function')
    module.exports = factory(require("jquery"), require("core/utils/type"), require("../../../helpers/executeAsyncMock.js"), require("../../../helpers/keyboardMock.js"), require("data/data_source/data_source"), require("data/array_store"), require("ui/list"));
  else
    throw new Error("Module must be loaded as AMD or CommonJS");
});
//# sourceMappingURL=editingTests.js.map