...
(["testing/tests/DevExpress.jquery/selectors.tests.js"], ["jquery","ui/widget/selectors"], true, function($__System) {
...
define("testing/tests/DevExpress.jquery/selectors.tests.js", ["require", "jquery", "ui/widget/selectors"], function(require) {
  const $ = require('jquery');
  if (QUnit.urlParams['nojquery']) {
    return;
  }
  QUnit.testStart(function() {
    const markup = `<div class="focusable tabbable" tabindex="0"></div>
  ...
  QUnit.test('tabbable', function(assert) {
    const focusableContainer = $('.tabbable');
    focusableContainer.each(function(index, item) {
      assert.ok($(item).is(selectors.tabbable));
    });
  });
  QUnit.test('not focusable', function(assert) {
    const focusableContainer = $('.notfocusable, .hidden-display, .hidden-visibility');
    focusableContainer.each(function(index, item) {
      assert.ok(!$(item).is(selectors.focusable));
    });
  });
  QUnit.test('not tabbable', function(assert) {
    const focusableContainer = $('.nottabbable, .hidden-display, .hidden-visibility');
    focusableContainer.each(function(index, item) {
      assert.ok(!$(item).is(selectors.tabbable));
    });
  });
...
(function(factory) {
  if (typeof define == 'function' && define.amd)
    define(["jquery","ui/widget/selectors"], factory);
  else if (typeof module == 'object' && module.exports && typeof require == 'function')
    module.exports = factory(require("jquery"), require("ui/widget/selectors"));
  else
    throw new Error("Module must be loaded as AMD or CommonJS");
});
//# sourceMappingURL=selectors.tests.js.map