...
(["testing/tests/DevExpress.knockout/componentRegistration.tests.js"], ["jquery","core/utils/common","knockout","core/component_registrator","core/dom_component","ui/widget/ui.widget","integration/knockout/template","ui/collection/ui.collection_widget.edit","core/config","core/element_data","ui/select_box","ui/lookup","integration/knockout"], true, function($__System) {
var require = this.require, exports = this.exports, module = this.module;
$__System.registerDynamic('testing/tests/DevExpress.knockout/componentRegistration.tests.js', ['jquery', 'core/utils/common', 'knockout', 'core/component_registrator', 'core/dom_component', 'ui/widget/ui.widget', 'integration/knockout/template', 'ui/collection/ui.collection_widget.edit', 'core/config', 'core/element_data', 'ui/select_box', 'ui/lookup', 'integration/knockout'], true, function ($__require, exports, module) {
    ...
    const DOMComponent = $__require('core/dom_component');
    const Widget = $__require('ui/widget/ui.widget');
    const KoTemplate = $__require('integration/knockout/template').KoTemplate;
    ...
    moduleWithoutCsp('simple component tests', {
        beforeEach: function () {
        ...
        QUnit.test('basic binding', function (assert) {
            const markup = $('<div></div>').attr('data-bind', 'dxTest: { text: text }').appendTo(FIXTURE_ELEMENT);
        ...
        QUnit.test('ignore dependent option after option changed (T335759)', function (assert) {
            ...
            const markup = $('<div>').attr('data-bind', 'dxTest: {obj: obj, onOptionChanged: optionChangedHandler}').appendTo(FIXTURE_ELEMENT);
        ...
        QUnit.test('nested option binding', function (assert) {
            const markup = $('<div></div>').attr('data-bind', 'dxTest: { complex: { first: { second: value } } }').appendTo(FIXTURE_ELEMENT);
        ...
        QUnit.test('nested viewmodel value binding', function (assert) {
            const markup = $('<div></div>').attr('data-bind', 'dxTest: { text: complex.nested }').appendTo(FIXTURE_ELEMENT);
        ...
        QUnit.test('nested viewmodel value and option binding', function (assert) {
            const markup = $('<div></div>').attr('data-bind', 'dxTest: { complex: complex }').appendTo(FIXTURE_ELEMENT);
        ...
        QUnit.test('nested viewModel value should not be unwrapped for reference option', function (assert) {
            const markup = $('<div></div>').attr('data-bind', 'dxTest: { complexByReference: complexByReference }').appendTo(FIXTURE_ELEMENT);
        ...
        QUnit.test('foreach binding', function (assert) {
            const vm = {
                items: ko.observableArray([{ text: ko.observable('0') }, { text: ko.observable('1') }])
            };

            const markup = $('<div>' + '   <!-- ko foreach: items -->' + '       <div data-bind=\'dxTest: { text: text }\'></div>' + '   <!-- /ko -->' + '</div>').appendTo(FIXTURE_ELEMENT);
        ...
        QUnit.test('DOMComponent does not control descendant bindings', function (assert) {
            const markup = $('<div data-bind=\'dxTest: { }\'>' + '   <ul data-bind=\'foreach: items\'>' + '       <li data-bind=\'text: $data\'></li>' + '   </ul>' + '</div>').appendTo(FIXTURE_ELEMENT);
        ...
        QUnit.test('circular notifications regression', function (assert) {
            ...
            const markup = $('<div></div>').attr('data-bind', 'dxTest: { text: scalar, array: array }').appendTo(FIXTURE_ELEMENT);
            ...
            vm.scalar.subscribe(function () {
                vmScalarChangeCount++;
            }, undefined, 'beforeChange');
            vm.array.subscribe(function () {
                vmArrayChangeCount++;
            }, undefined, 'beforeChange');
        ...
        QUnit.test('dependency from outer observable', function (assert) {
            const markup = $('<div></div>').attr('data-bind', 'dxTest: { text: a().b().c }').appendTo(FIXTURE_ELEMENT);
        ...
        QUnit.test('model property name is not necessarily equal component option name, regression case', function (assert) {
            const markup = $('<div></div>').attr('data-bind', 'dxTest: { text: textInModel }').appendTo(FIXTURE_ELEMENT);
        ...
        QUnit.test('remove from DOM, append back and apply bindings again', function (assert) {
            ...
            markup.remove().appendTo(FIXTURE_ELEMENT);
            ko.applyBindings(vm, markup[0]);
            vm.text('new value');
        ...
        QUnit.test('B239952 - simple case', function (assert) {
            const markup = $('<div><div />').attr('data-bind', 'dxTest: { option1: field1, option2: field2 }').appendTo(FIXTURE_ELEMENT);
        ...
        QUnit.test('B239952 - complex case', function (assert) {
            const markup = $('<div><div />').attr('data-bind', 'dxTest: { option1: field1, option2: field2, option3: field3 }').appendTo(FIXTURE_ELEMENT);
        ...
        QUnit.test('batch option changing involves several component re-rendering (B251357)', function (assert) {
            ...
            const markup = $('<div><div />').attr('data-bind', 'dxTest: { option1: componentOptions().option1, option2: componentOptions().option1, option3: componentOptions().option1 }').appendTo(FIXTURE_ELEMENT);
        ...
        QUnit.test('expression with nested observable binding (T120420)', function (assert) {
            const markup = $('<div><div />').attr('data-bind', 'dxTest: { option1: point() ? point().x : 0 }').appendTo(FIXTURE_ELEMENT);
        ...
        QUnit.test('component should works correctly if observable option changed on ctor (T179839)', function (assert) {
            ...
            const markup = $('<div><div />').attr('data-bind', 'dxTestWidget: { option1: myValue, onContentReady: onContentReady }').appendTo(FIXTURE_ELEMENT);
        ...
        QUnit.test('component should not fire option changed callbacks after disposing (T215559)', function (assert) {
            ...
            const markup = $('<div data-bind=\'if: visible\'>\
        ...
        QUnit.test('our component binding handler should subscribe correctly on get/set properties (T257177)', function (assert) {
            ...
            const callArgs = [];
            $component.T257177('instance').on('optionChanged', function (a) {
                callArgs.push(a);
            });
    ...
    moduleWithoutCsp('nested Widget with templates enabled', {
        beforeEach: function () {
            const TestContainer = Widget.inherit({
                ...
                _render: function () {
                    const content = $('<div />').addClass('dx-content').appendTo(this.$element());
        ...
        QUnit.test('two nested containers', function (assert) {
            const markup = $('<div class=outerWidget data-bind=\'dxTestContainer: { }\'>' + '   <div data-options=\'dxTemplate: { name: "template" }\' class=outer-template>' + '       <span data-bind=\'text: outerText\'></span>' + '       <div class=innerWidget data-bind=\'dxTestContainer: { }\'>' + '           <div data-options=\'dxTemplate: { name: "template" }\' >' + '               <span data-bind=\'text: innerText\'></span>' + '           </div>' + '       </div>' + '   </div>' + '</div>').appendTo(FIXTURE_ELEMENT);
        ...
        QUnit.test('B233347, B233267', function (assert) {
            const markup = $('<div class=outerWidget data-bind=\'dxTestContainer: { }\'>' + '   <div data-options=\'dxTemplate: { name: "template" }\'>' + '       <span class=template-content data-bind=\'text: text\'></span>' + '   </div>' + '</div>').appendTo(FIXTURE_ELEMENT);
        ...
        QUnit.test('widget inside two nested containers', function (assert) {
            const markup = $('<div data-bind=\'dxTestContainer: { text: outerText }\'>' + '   <div class=middle data-bind=\'dxTestContainer: { text: middleText }\'>' + '       <div class=inner data-bind=\'dxTestWidget: { text: innerText }\'></div>' + '   </div>' + '</div>').appendTo(FIXTURE_ELEMENT);
    ...
    moduleWithoutCsp('Widget & CollectionWidget with templates enabled', function () {
        ...
        QUnit.test('default KO template is not retrieved for widgets created with knockout', function (assert) {
            ...
            try {
                ...
                // jquery scenario
                markup = $('<div></div>').appendTo(FIXTURE_ELEMENT).dxTestContainer({});
                instance = markup.dxTestContainer('instance');
                template = instance._getTemplate('test');
        ...
        QUnit.test('creates default template from its contents', function (assert) {
            ...
            try {
                ...
                const markup = $('<div data-bind=\'dxTestContainer: { items: items }\'>' + '   <ul data-bind=\'foreach: items\'>' + '       <li data-bind=\'text: $data\'></li>' + '   </ul>' + '</div>').appendTo(FIXTURE_ELEMENT);
        ...
        QUnit.test('binding-context of KoTemplate', function (assert) {
            ...
            try {
                const markup = $('<div data-bind=\'dxTestRepeater: { items: items }\'>' + '    <div data-options=\'dxTemplate: { name: ' + '"itemTemplate"' + '}\'>' + '        <div class=item data-bind=\'text: $data\'></div>' + '    </div>' + '</div>').appendTo(FIXTURE_ELEMENT);
    ...
    moduleWithoutCsp('component disposing on node removing', {
        beforeEach: function () {
        ...
        QUnit.test('ko.observable subscriptions should be disposed', function (assert) {
            ...
            let markup = $('<div></div>').attr('data-bind', 'dxTest: { text: text }').appendTo(FIXTURE_ELEMENT);
            ...
            markup = $('<div>' + '   <!-- ko foreach: items -->' + '       <div data-bind=\'dxTest: { text: text }\'></div>' + '   <!-- /ko -->' + '</div>').appendTo(FIXTURE_ELEMENT);
        ...
        $.each({
            ...
            QUnit.test('regression: B233447 - component disposing should be called once per component (' + caseName + ')', function (assert) {
                registerComponent('dxTestComponent', DOMComponent.inherit({
                    _dispose: function () {
        ...
        QUnit.test('CollectionWidget clean subscriptions', function (assert) {
            ...
            const markup = $('<div data-bind=\'dxTestCollectionContainer: { items: items }\'>' + '</div>').appendTo(FIXTURE_ELEMENT);
    ...
    moduleWithoutCsp('component action context', {
        beforeEach: function () {
            ...
            $('<div id=\'aggregatedModelComponentActionContext\'>' + '<!-- ko with: subVM -->' + '<div class=\'test\' data-bind=\'dxTest: { onHandler: handler }\'></div>' + '<!-- /ko -->' + '</div>').appendTo(FIXTURE_ELEMENT);
        }
    }, function () {
    ...
    moduleWithoutCsp('Template w/o ko scenario', function () {
        ...
        QUnit.test('widget with templates enabled', function (assert) {
            ...
            try {
                const $markup = $('<div data-bind=\'dxTestContainer: { }\'>' + '    <div class=\'content\'>Test</div>' + '</div>').appendTo(FIXTURE_ELEMENT);
        ...
        QUnit.test('collection container widget', function (assert) {
            ...
            try {
                const $markup = $('<div data-bind=\'dxTestCollectionContainer: { items: items }\'>' + '    <div data-options=\'dxTemplate: {name: "item"}\'>' + '        <div>Test</div>' + '    </div>' + '</div>').appendTo(FIXTURE_ELEMENT);
    ...
    moduleWithoutCsp('predicate for manual option binding control', {
        beforeEach: function () {
            ...
            registerComponent('dxTest', DOMComponent.inherit({ _useTemplates() {
                    return false;
                } }));

            this.$component = $('<div></div>').attr('data-bind', 'dxTest: $data').appendTo(FIXTURE_ELEMENT);
        },
        afterEach: function () {
        ...
        QUnit.test('arguments are \'propertyPath\', \'propertyName\', \'currentModel\'', function (assert) {
            const vm = {
                isBindingProperty: sinon.spy(function () {
                    return true;
                }),
            ...
            assert.deepEqual(vm.isBindingProperty.args, [['objectProperty', 'objectProperty', vm], ['objectProperty.nestedProperty', 'nestedProperty', vm.objectProperty]]);
        });
...
(function(factory) {
  if (typeof define == 'function' && define.amd)
    define(["jquery","core/utils/common","knockout","core/component_registrator","core/dom_component","ui/widget/ui.widget","integration/knockout/template","ui/collection/ui.collection_widget.edit","core/config","core/element_data","ui/select_box","ui/lookup","integration/knockout"], factory);
  else if (typeof module == 'object' && module.exports && typeof require == 'function')
    module.exports = factory(require("jquery"), require("core/utils/common"), require("knockout"), require("core/component_registrator"), require("core/dom_component"), require("ui/widget/ui.widget"), require("integration/knockout/template"), require("ui/collection/ui.collection_widget.edit"), require("core/config"), require("core/element_data"), require("ui/select_box"), require("ui/lookup"), require("integration/knockout"));
  else
