System.register(["jquery", "core/config", "format_helper", "ui/widget/ui.errors", "data/errors", "core/utils/type", "data/data_source/data_source", "data/array_store", "ui/grid_core/ui.grid_core.utils", "../../helpers/dataGridMocks.js", "ui/data_grid"], function (_export, _context) {
  "use strict";

  var $, config, formatHelper, errors, dataErrors, typeUtils, DataSource, ArrayStore, gridCoreUtils, setupDataGridModules, MockGridDataSource, TEN_NUMBERS, rowsViewMock, createDataSource, setupModule, teardownModule, setupPagingModule, teardownPagingModule, setupVirtualScrollingModule, teardownVirtualScrollingModule, setupVirtualRenderingModule, teardownVirtualRenderingModule;
  return {
    setters: [function (_jquery) {
      $ = _jquery.default;
    }, function (_coreConfig) {
      config = _coreConfig.default;
    }, function (_format_helper) {
      formatHelper = _format_helper.default;
    }, function (_uiWidgetUiErrors) {
      errors = _uiWidgetUiErrors.default;
    }, function (_dataErrors) {
      dataErrors = _dataErrors.errors;
    }, function (_coreUtilsType) {
      typeUtils = _coreUtilsType.default;
    }, function (_dataData_sourceData_source) {
      DataSource = _dataData_sourceData_source.DataSource;
    }, function (_dataArray_store) {
      ArrayStore = _dataArray_store.default;
    }, function (_uiGrid_coreUiGrid_coreUtils) {
      gridCoreUtils = _uiGrid_coreUiGrid_coreUtils.default;
    }, function (_helpersDataGridMocksJs) {
      setupDataGridModules = _helpersDataGridMocksJs.setupDataGridModules;
      MockGridDataSource = _helpersDataGridMocksJs.MockGridDataSource;
    }, function (_uiData_grid) {}],
    execute: function () {
      ...
      createDataSource = function (data, storeOptions, dataSourceOptions) {
        const arrayStore = new ArrayStore(storeOptions ? $.extend(true, {
          data: data
        }, storeOptions) : data);
        const dataSource = new DataSource($.extend(true, {
          store: arrayStore,
      ...
      setupModule = function () {
        setupDataGridModules(this, ['data', 'virtualScrolling', 'columns', 'filterRow', 'search', 'editing', 'editingRowBased', 'editingFormBased', 'editingCellBased', 'grouping', 'headerFilter', 'masterDetail', 'editorFactory', 'focus', 'keyboardNavigation', 'summary', 'selection', 'virtualColumns']);
        ...
        this.editingController.getFirstEditableCellInRow = function () {
          return $([]);
        };
      };
      ...
      QUnit.module('Initialization', {
        ...
        QUnit.test('Initialize from dataSource with all visible columns', function (assert) {
          ...
          assert.deepEqual(this.dataController.items()[1].data, {
            name: 'Dan',
            phone: '98-75-21'
          });
        });
        ...
        QUnit.test('The pushed callback on initialize', function (assert) {
          ...
          assert.deepEqual(pushedSpy.getCall(0).args[0], [{
            type: 'remove',
            key: 1
          }], 'the pushed callback args');
        });
        QUnit.test('The handler of the dataSource pushed callback should be removed after disposing dataSource', function (assert) {
          ...
          let dataSource = createDataSource(array, {
            key: 'id'
          });
        ...
        QUnit.test('set isExpanded group parameters to dataSource on initialization', function (assert) {
          ...
          // assert
          assert.deepEqual(this.dataController._dataSource.group(), [{
            selector: 'name',
            isExpanded: true,
            desc: false
          }, {
        ...
        QUnit.test('grouping with autoExpandAll is false', function (assert) {
          ...
          assert.equal(this.dataController.items()[0].data.key, 'Alex');
          assert.deepEqual(this.dataController.items()[0].data.collapsedItems, [{
            name: 'Alex',
            age: 30
          }]);
        ...
        QUnit.test('collapseAll when remoteOperations with grouping and without paging', function (assert) {
          ...
          assert.deepEqual(loadingArgs[0].group, [{
            selector: 'name',
            isExpanded: true,
            desc: false
          }], 'loading group arg');
        ...
        QUnit.test('Using focusedRowEnabled should not set sorting for the not sorted composite key columns', function (assert) {
          ...
          assert.deepEqual(this.dataController._columnsController.getSortDataSourceParameters(), [{
            desc: false,
            selector: dataIndexGetter
          }], 'Sort parameters');
          // assert
          assert.equal(dataSource.items()[0].name, 'Alex', 'Item0');
          assert.equal(dataSource.items()[1].name, 'Dan', 'Item1');
        ...
        QUnit.test('Refresh after reordering should update rows if focusedRowEnabled is enabled', function (assert) {
          ...
          const dataSource = createDataSource(items, {
            key: 'name'
          });
          this.applyOptions({
            dataSource: dataSource,
        ...
        QUnit.test('Check the filter generator for the boolean field', function (assert) {
          // act, assert
          let filter = this.dataController._generateBooleanFilter('isRoom', true, {
            desc: false
          });
          assert.strictEqual(JSON.stringify(filter), '["isRoom","<>",true]', 'filter');

          // act, assert
          filter = this.dataController._generateBooleanFilter('isRoom', false, {
            desc: false
          });
          assert.strictEqual(JSON.stringify(filter), '["isRoom","=",null]', 'filter');

          // act, assert
          filter = this.dataController._generateBooleanFilter('isRoom', true, {
            desc: true
          });
          ...
          // act, assert
          filter = this.dataController._generateBooleanFilter('isRoom', null, {
            desc: true
          });
          assert.strictEqual(JSON.stringify(filter), '["isRoom","<>",null]', 'filter');
        ...
        QUnit.test('Get page index by composite key', function (assert) {
          ...
          // act
          this.dataController._refreshDataSource();
          this.dataController.getPageIndexByKey({
            name: 'Bob',
            age: 25
          }).done(function (pageIndex) {
            assert.equal(pageIndex, 1);
          });
        });
        ...
        QUnit.test('Get page index by composite key if combined filter present', function (assert) {
          ...
          // assert
          dataController.getPageIndexByKey({
            name: 'Bob',
            age: 24
          }).done(function (pageIndex) {
        ...
        QUnit.test('Reset sort/group parameters from user state for removed columns', function (assert) {
          ...
          // assert
          assert.deepEqual(this.dataController._dataSource.sort(), [{
            selector: 'age',
            desc: false
          }], 'sort from user state is applied by dataField');
        ...
        QUnit.test('changed when ArrayStore with datetime fields in string format and sorting defined and serializationFormat defined', function (assert) {
          ...
          const dataSource = createDataSource([{
            name: 'Alex',
            birthDate: '1987-05-05T00:00:00Z'
          }, {
            name: 'Dan',
            birthDate: '1985-03-21T00:00:00Z'
          }], {}, {
            asyncLoadEnabled: true
          });
          this.applyOptions({
            columns: ['Alex', {
              dataField: 'birthDate',
              dataType: 'date',
              serializationFormat: 'yyyy-MM-ddTHH:mm:ss\'Z\'',
              sortOrder: 'asc'
            }]
          });
          dataController.changed.add(function () {
      ...
      teardownPagingModule = function () {
        teardownModule.apply(this);
      };
      QUnit.module('Paging', {
        beforeEach: setupPagingModule,
        afterEach: teardownPagingModule
      }, () => {
      ...
      setupVirtualScrollingModule = function () {
        this.options = {
          scrolling: {
            mode: 'virtual'
          },
      ...
      teardownVirtualScrollingModule = function () {
        teardownModule.apply(this);
      };
      QUnit.module('Virtual scrolling', {
        beforeEach: setupVirtualScrollingModule,
        afterEach: teardownVirtualScrollingModule
      }, () => {
        ...
        QUnit.test('rows should not be recreated on pageIndex event', function (assert) {
          ...
          // act
          this.dataController.dataSource().changed.fire({
      ...
      setupVirtualRenderingModule = function () {
        const array = [];
        for (let i = 0; i < 100; i++) {
          array.push({
            id: i,
      ...
      teardownVirtualRenderingModule = function () {
        this.dispose();
        this.clock.restore();
      };
      QUnit.module('Virtual rendering', {
        beforeEach: setupVirtualRenderingModule,
        afterEach: teardownVirtualRenderingModule
      }, () => {
        ...
        QUnit.test('scroll to second render page', function (assert) {
          ...
          assert.deepEqual(this.changedArgs, [{
            ...
            changeTypes: ['remove', 'remove', 'remove', 'remove', 'remove', 'insert', 'insert', 'insert', 'insert', 'insert'],
            columnIndices: [undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined],
            operationTypes: {
              filtering: false,
              fullReload: false,
        ...
        QUnit.test('scroll to second render page after expand row on the first page', function (assert) {
          ...
          assert.deepEqual(this.changedArgs[0].changeTypes, ['remove', 'remove', 'remove', 'remove', 'remove', 'remove', 'insert', 'insert', 'insert', 'insert', 'insert']);
        ...
        QUnit.test('scroll to second render page and return to first after expand row on the first page', function (assert) {
          ...
          assert.deepEqual(this.changedArgs[0].changeType, 'update');
          assert.deepEqual(this.changedArgs[0].changeTypes, ['insert', 'insert', 'insert', 'insert', 'insert', 'insert', 'remove', 'remove', 'remove', 'remove', 'remove']);
        ...
        QUnit.test('scroll to second dataSource page', function (assert) {
          ...
          assert.deepEqual(this.changedArgs[0].changeTypes, ['remove', 'remove', 'remove', 'remove', 'remove', 'remove', 'remove', 'remove', 'remove', 'remove', 'insert', 'insert', 'insert', 'insert', 'insert', 'insert', 'insert', 'insert', 'insert', 'insert']);
        ...
        QUnit.test('scroll to previous render page', function (assert) {
          ...
          assert.deepEqual(this.changedArgs[0].changeTypes, ['insert', 'insert', 'insert', 'insert', 'insert', 'remove', 'remove', 'remove', 'remove', 'remove']);
        ...
        QUnit.test('scroll to second page > add row > scroll back > scroll to second page', function (assert) {
          ...
          // assert
          assert.deepEqual(changedStub.getCalls().map(c => c.args[0]).filter(e => e.changeType !== 'pageIndex').map(({
            changeType,
            changeTypes
          }) => ({
            changeType,
      ...
      QUnit.module('Virtual scrolling (ScrollingDataSource)', {
        beforeEach: function () {
          ...
          this.setupDataSource = function (options) {
            ...
            this.dataSource = createDataSource(options.data || TEN_NUMBERS, {}, $.extend({
              paginate: true
            }, options));
            this.dataController.setDataSource(this.dataSource);
            this.dataSource.load();
        ...
        QUnit.test('update loading when grouping', function (assert) {
          const array = $.map(TEN_NUMBERS, function (value) {
            return {
              value: value
            };
          });
        ...
        QUnit.test('changeRowExpand on second page when virtual scrolling enabled', function (assert) {
          ...
          assert.deepEqual(dataController.items()[0].data, {
            collapsedItems: [],
            key: 4,
            items: null,
            count: 1
        ...
        QUnit.test('New mode. Load params are synchronized after scrolling', function (assert) {
          ...
          // assert
          assert.deepEqual(this.dataController.getLoadPageParams(), {
            pageIndex: 2,
            loadPageCount: 4,
            skipForCurrentPage: 5
          }, 'load page params after scrolling');
          ...
          assert.deepEqual(loadedItems[0], {
            id: 21,
            name: 'Name 21'
          }, 'first loaded item');
          assert.deepEqual(loadedItems[39], {
            id: 60,
            name: 'Name 60'
          }, 'last loaded item');
          assert.equal(visibleItems.length, 16, 'visible items count');
          assert.deepEqual(visibleItems[0].data, {
            id: 26,
            name: 'Name 26'
          }, 'first visible item');
          assert.deepEqual(visibleItems[15].data, {
            id: 41,
            name: 'Name 41'
          }, 'last visible item');
        });
      ...
      QUnit.module('Virtual scrolling preload', {
        ...
        QUnit.test('New mode. Load page params should not be changed if the viewport position is not changed after scrolling back (T1052705)', function (assert) {
          ...
          // assert
          assert.deepEqual(loadPageParams, {
            pageIndex: 2,
            loadPageCount: 3,
            skipForCurrentPage: 10
          }, 'params after scrolling down');
          ...
          assert.deepEqual(loadPageParams, {
            pageIndex: 1,
            loadPageCount: 2,
            skipForCurrentPage: 15
          }, 'params after scrolling up');
      ...
      QUnit.module('Filtering', {
        ...
        QUnit.test('get filter and combinedFilter', function (assert) {
          ...
          assert.deepEqual(this.getCombinedFilter(), [[[['name', 'contains', 'A'], 'and', ['age', '=', 15]], 'and', ['name', 'contains', 'Al']], 'and', ['name', '=', 'Alex']]);
        });
        ...
        QUnit.test('get combinedFilter for search when allowSearch false', function (assert) {
          ...
          // act
          assert.deepEqual(this.filter(), ['name', '=', 'Alex']);
          assert.deepEqual(this.getCombinedFilter(), [['age', 'contains', 'Al'], 'and', ['name', '=', 'Alex']]);
        });
        QUnit.test('get combinedFilter for search when searchVisibleColumnsOnly true and column is not visible', function (assert) {
          ...
          // assert
          assert.deepEqual(this.filter(), ['name', '=', 'Alex']);
          assert.deepEqual(this.getCombinedFilter(), [[['name', 'contains', 'Al'], 'or', ['age', 'contains', 'Al']], 'and', ['name', '=', 'Alex']]);
        ...
        QUnit.test('get combinedFilter when filterRow between operation and headerFilter is used', function (assert) {
          ...
          // act
          assert.deepEqual(this.getCombinedFilter(), [[['age', '>=', 15], 'and', ['age', '<=', 20]], 'and', ['age', '=', 17]]);
        });
        ...
        QUnit.test('get combinedFilter with filter on only one column', function (assert) {
          ...
          // assert
          assert.deepEqual(this.filter(), ['name', '=', 'Alex']);
          assert.deepEqual(this.getCombinedFilter(), [['age', '=', 15], 'and', ['name', '=', 'Alex']]);
        });
        ...
        QUnit.test('clearFilter for dataSource', function (assert) {
          ...
          that.applyOptions({
            columns: [{
              dataField: 'name',
              filterValues: ['Alex', 'Dan', 'Bob', 'Bobbi']
            }, {
              dataField: 'age',
              filterValue: 19
            }]
        ...
        QUnit.test('column filter for column with calculateFilterExpression returns filter function when cache enabled', function (assert) {
          ...
          this.columnsController.columnOption(3, 'calculateFilterExpression', function (text, operation) {
            return function (data) {
              return (data.name + ' ' + data.age).indexOf(text) >= 0;
            };
          });
          this.columnsController.columnOption(3, 'filterValue', '9');
        ...
        QUnit.test('Not apply groupInterval of the headerFilter for filterRow', function (assert) {
          ...
          this.dataSource = createDataSource([{
            name: 'Alex',
            birthDate: new Date(1992, 7, 6, 12, 30, 21)
          }]);
          this.applyOptions({
            columns: ['name', {
              dataField: 'birthDate',
      ...
      QUnit.module('Editing', {
        ...
        QUnit.test('Inserting Row for grouped data', function (assert) {
          ...
          assert.deepEqual(this.dataController.items()[2].data, {
            name: 'Alex',
            phone: '55-55-55'
          }, 'item 2 data');
      ...
      QUnit.module('Remote Grouping', {
        ...
        QUnit.test('Error when CustomStore returns plain data and remote grouping enabled', function (assert) {
          ...
          assert.deepEqual(storeLoadOptions.group, [{
            selector: 'name',
            desc: false,
            isExpanded: false
          }], 'group option');
      ...
      QUnit.module('Summary', {
        ...
        QUnit.test('CustomStore load options when remote summary enabled', function (assert) {
          ...
          assert.strictEqual(storeLoadOptions.skip, 0, 'skip option');
          assert.strictEqual(storeLoadOptions.take, 2, 'take option');
          assert.deepEqual(storeLoadOptions.totalSummary, [{
            selector: 'age',
            summaryType: 'custom'
          }], 'totalSummary option');
          assert.ok(!this.dataController.isLoading());
        ...
        QUnit.test('CustomStore load options when remote summary and remote grouping without paging enabled', function (assert) {
          ...
          assert.deepEqual(storeLoadOptions.groupSummary, [{
            selector: 'age',
            summaryType: 'count'
          }], 'summary groupItems option');
          assert.ok(!this.dataController.isLoading());
          assert.equal(this.dataController.totalCount(), 2, 'totalCount');
          assert.equal(this.dataController.pageCount(), 1, 'pageCount');
          assert.deepEqual(this.dataController.items()[0].summaryCells, [[], [{
        ...
        QUnit.test('CustomStore load options when remote operations enabled and multi-grouping with sort by summary info is defined', function (assert) {
          ...
          assert.strictEqual(storeLoadOptions.skip, undefined, 'no skip option');
          assert.strictEqual(storeLoadOptions.take, undefined, 'no take option');
          assert.deepEqual(storeLoadOptions.filter, [[['group1', '=', 'Group1'], 'and', ['group2', '=', 'Group1_0']], 'or', [['group1', '=', 'Group0'], 'and', ['group2', '=', 'Group0_0']]], 'filter option');
          assert.deepEqual(storeLoadOptions.sort, [{
            'desc': false,
            'isExpanded': true,
            'selector': 'group1'
        ...
        QUnit.test('Changing sortByGroupSummaryInfo', function (assert) {
          ...
          // act
          this.option('sortByGroupSummaryInfo', [{
            summaryItem: 'count'
          }]);
          this.clock.tick(10);
        ...
        QUnit.test('headerFilter items if sortByGroupSummaryInfo is defined', function (assert) {
          ...
          // assert
          assert.deepEqual(headerFilterItems.map(function (item) {
            return item.value;
          }), [15, 18, 19, 20, 25], 'items are sorted');
        });
        ...
        QUnit.test('group summary item alignByColumn', function (assert) {
          ...
          assert.deepEqual(this.dataController.items()[0].summaryCells, [[], [], [$.extend({
            value: 15
          }, this.options.summary.groupItems[0])]]);
          assert.strictEqual(this.dataController.items()[3].rowType, 'group');
          assert.deepEqual(this.dataController.items()[3].summaryCells, [[], [], [$.extend({
            value: 25
          }, this.options.summary.groupItems[0])]]);
        });
        QUnit.test('group summary items with and without alignByColumn', function (assert) {
          ...
          assert.deepEqual(this.dataController.items()[0].summaryCells, [[], [$.extend({
            value: 2
          }, this.options.summary.groupItems[0])], [$.extend({
            value: 15
          }, this.options.summary.groupItems[1])]]);
          assert.strictEqual(this.dataController.items()[3].rowType, 'group');
          assert.deepEqual(this.dataController.items()[3].summaryCells, [[], [$.extend({
            value: 1
          }, this.options.summary.groupItems[0])], [$.extend({
        ...
        QUnit.test('group summary item alignByColumn for first cell after group', function (assert) {
          ...
          assert.deepEqual(this.dataController.items()[1].summaryCells, [[], [], [$.extend({
            value: 34,
            columnCaption: 'Age'
          }, this.options.summary.groupItems[0])]]);
          assert.strictEqual(this.dataController.items()[4].rowType, 'group');
          assert.deepEqual(this.dataController.items()[4].summaryCells, [[], [], [$.extend({
            value: 25,
            columnCaption: 'Age'
          }, this.options.summary.groupItems[0])]]);
        });
        ...
        QUnit.test('groupFooter item when group summary items with showInGroupFooter and autoExpandAll', function (assert) {
          ...
          assert.deepEqual(this.dataController.items()[6].summaryCells, [[], [$.extend({
            value: 25
          }, this.options.summary.groupItems[0])]]);
        });
        ...
        beforeEach: function () {
          ...
          this.getTotalValues = function () {
            return this.dataController.footerItems()[0].summaryCells.map(function (summaryItems) {
              return summaryItems.length <= 1 ? summaryItems[0] && summaryItems[0].value : summaryItems.map(function (item) {
                return item.value;
              });
            });
          };
      ...
      QUnit.module('Partial update', {
        beforeEach: function () {
          ...
          that.setupModules = function () {
            ...
            that.dataSource = createDataSource(that.array, {
              key: 'name'
            });
            that.dataController.setDataSource(that.dataSource);
            that.dataSource.load();
      ...
      QUnit.module('Refresh changesOnly', {
        ...
        QUnit.test('focused row index should be corrected after insert', function (assert) {
          ...
          assert.deepEqual(this.keyboardNavigationController._focusedCellPosition, {
            rowIndex: 1,
            columnIndex: 1
          }, 'focused row index is corrected');
        });
        ...
        QUnit.test('immutable change dataSource item field', function (assert) {
          ...
          changedArray[1] = $.extend({}, changedArray[1], {
            age: 99
          });
          this.dataController.optionChanged({
            name: 'dataSource',
        ...
        QUnit.test('change dataSource to another type', function (assert) {
          ...
          this.option('dataSource', createDataSource(this.array.slice(1), {
            key: 'id'
          }));
      ...
      QUnit.module('Using DataSource instance', {
        ...
        QUnit.test('Grouping not reset after change option autoExpandAll', function (assert) {
          ...
          assert.deepEqual(this.dataController._dataSource.group(), [{
            ...
            desc: false,
            isExpanded: true
          }], 'dataSource group when autoExpandAll true');
          ...
          assert.deepEqual(this.dataController._dataSource.group(), [{
            ...
            desc: false,
            isExpanded: false
          }], 'dataSource group when autoExpandAll false');
      ...
      QUnit.module('Exporting', {
        ...
        QUnit.test('loadAll when remote summary enabled', function (assert) {
          ...
          assert.deepEqual(loadArgs[1].totalSummary, [{
            selector: 'field2',
            summaryType: 'sum'
          }], 'loadAll totalSummary');
        ...
        QUnit.test('loadAll when remote grouping and summary enabled', function (assert) {
          ...
          assert.deepEqual(allItems[0].data, {
            key: 1,
            count: 1,
            items: [{
              field1: 1,
          ...
          assert.deepEqual(allItems[1].data, {
            field1: 1,
            field2: 2
          }, 'item 1');
          assert.deepEqual(allItems[2].data, {
            key: 3,
            count: 1,
            items: [{
              field1: 3,
          ...
          assert.deepEqual(allItems[3].data, {
            field1: 3,
            field2: 4
          }, 'item 3');
          ...
          assert.deepEqual(loadArgs[0].groupSummary, [{
            selector: 'field2',
            summaryType: 'sum'
          }], 'initial load groupSummary');
          assert.deepEqual(loadArgs[1].skip, undefined, 'loadAll skip');
          assert.deepEqual(loadArgs[1].take, undefined, 'loadAll load take');
          ...
          assert.deepEqual(loadArgs[1].groupSummary, [{
            selector: 'field2',
            summaryType: 'sum'
          }], 'loadAll groupSummary');
          ...
          assert.deepEqual(loadArgs[2].totalSummary, [{
            selector: 'field1',
            summaryType: 'sum'
          }], 'loadAll totalSummary');
          assert.deepEqual(loadArgs[2].groupSummary, [{
            selector: 'field2',
            summaryType: 'sum'
          }], 'loadAll groupSummary');
        });
        ...
        QUnit.test('loadAll by selected items when remote grouping and summary enabled', function (assert) {
          ...
          // act
          loadArgs = [];
          this.dataController.loadAll([{
            field1: 1,
            field2: 2
          }]).done(function (items, summary) {
            allItems = items;
            allSummary = summary;
          });
          this.clock.tick(10);
