...
(["testing/tests/DevExpress.animation/fx.tests.js"], ["jquery","core/renderer","events/core/events_engine","animation/fx","animation/translator","animation/frame","core/utils/support","animation/position"], true, function($__System) {
var require = this.require, exports = this.exports, module = this.module;
$__System.registerDynamic('testing/tests/DevExpress.animation/fx.tests.js', ['jquery', 'core/renderer', 'events/core/events_engine', 'animation/fx', 'animation/translator', 'animation/frame', 'core/utils/support', 'animation/position'], true, function ($__require, exports, module) {
    var global = this || self,
        GLOBAL = global;
    const $ = $__require('jquery');
    const renderer = $__require('core/renderer');
    const eventsEngine = $__require('events/core/events_engine');
    const fx = $__require('animation/fx');
    const translator = $__require('animation/translator');
    const animationFrame = $__require('animation/frame');
    const support = $__require('core/utils/support');
    const positionUtils = $__require('animation/position');
        ...
        QUnit.test('basic animation via CSS class', function (assert) {
            ...
            this.animate($element, {
                ...
                complete: function () {
                    ...
                    assert.equal($element.position().left, 1000);
                    setTimeout(function () {
                        // browser recalculate styles only during the next paint
                        assert.ok(!$element.is('.my-animation'));
                        assert.ok(!$element.is('.my-animation-active'));
                        assert.equal($element.position().left, 0);
                        done();
    ...
    QUnit.test('fade should change visibility before animation', function (assert) {
        ...
        const done = assert.async();
        const $element = $('#test').css('opacity', 0).css('visibility', 'hidden');
    ...
    QUnit.test('animations should be pushed to queue correctly on complete handler', function (assert) {
        ...
        d.done(function () {
            assert.deepEqual(log, ['first outer start', 'first outer complete', 'second outer start', 'second outer complete', 'first inner start', 'first inner complete', 'second inner start', 'second inner complete']);
            done();
        });
    });
...
(function(factory) {
  if (typeof define == 'function' && define.amd)
    define(["jquery","core/renderer","events/core/events_engine","animation/fx","animation/translator","animation/frame","core/utils/support","animation/position"], factory);
  else if (typeof module == 'object' && module.exports && typeof require == 'function')
    module.exports = factory(require("jquery"), require("core/renderer"), require("events/core/events_engine"), require("animation/fx"), require("animation/translator"), require("animation/frame"), require("core/utils/support"), require("animation/position"));
  else
