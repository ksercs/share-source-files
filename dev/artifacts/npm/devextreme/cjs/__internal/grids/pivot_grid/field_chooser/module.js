...
var compareOrder = [
    ...
    module_widget_utils_1.getCompareFunction(function (item) { return item.text; }),
];
...
var FieldChooser = module_base_1.FieldChooserBase.inherit({
    ...
    _initMarkup: function () {
        ...
        that.callBase();
        $element
            .addClass(const_1.CLASSES.fieldChooser.self)
    ...
    _createFieldsDataSource: function (dataSource) {
        ...
        fields = fields.filter(function (field) { return field.visible !== false && !type_1.isDefined(field.groupIndex); });
    ...
    _renderFieldsTreeView: function (container) {
        ...
        var treeView = that._createComponent(container, tree_view_1.default, {
            ...
            itemTemplate: function (itemData, itemIndex, itemElement) {
                var _a;
                var $item = renderer_1.default('<div>')
                    .toggleClass(const_1.CLASSES.area.field, !itemData.items)
                    .attr(const_1.ATTRIBUTES.treeViewItem, true)
                    .data('field', itemData.field)
                    .appendTo(itemElement);
                if (itemData.icon) {
                    (_a = icon_1.getImageContainer(itemData.icon)) === null || _a === void 0 ? void 0 : _a.appendTo($item);
                }
                renderer_1.default('<span>')
                    .text(itemData.text)
                    .appendTo($item);
            },
            ...
            onItemSelectionChanged: function (e) {
                ...
                if (data.items) {
                    ...
                    if (needSelectDefaultItem) {
                        var item = getFirstItem(data, function (item) { return item.isDefault; }) || getFirstItem(data, function (item) { return type_1.isDefined(item.index); });
    ...
    _renderArea: function (container, area) {
        ...
        var $fieldsHeaderContainer = renderer_1.default(DIV)
            .addClass(const_1.CLASSES.area.fieldListHeader)
            .appendTo($areaContainer);
        var caption = that.option("texts." + area + "Fields");
        var $fieldsContent;
        var render;
        renderer_1.default('<span>').addClass(const_1.CLASSES.area.icon)
            .addClass("dx-area-icon-" + area)
            .appendTo($fieldsHeaderContainer);
        renderer_1.default('<span>')
            .html('&nbsp;')
            .appendTo($fieldsHeaderContainer);
        renderer_1.default('<span>').addClass(const_1.CLASSES.area.caption)
            .text(caption)
            .appendTo($fieldsHeaderContainer);
        var $fieldsContainer = renderer_1.default(DIV).addClass(const_1.CLASSES.area.fieldList)
            .addClass(const_1.CLASSES.pivotGrid.dragAction)
            .appendTo($areaContainer);
        if (area !== 'all') {
...
exports.default = { FieldChooser: FieldChooser };
