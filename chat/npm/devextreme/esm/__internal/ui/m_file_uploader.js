...
class FileUploader extends Editor {
  ...
  _createFiles() {
    ...
    // @ts-expect-error
    each(value === null || value === void 0 ? void 0 : value.slice(this._files.length), (_, value) => {
  ...
  _validateFileExtension(file) {
    ...
    // @ts-expect-error
    if ((accept === null || accept === void 0 ? void 0 : accept.length) !== 0 && !this._isFileTypeAllowed(file.value, allowedTypes)) {
      return false;
    }
    // @ts-expect-error
  ...
  _renderFiles() {
    ...
    if (!this._$filesContainer) {
      this._$filesContainer = $('<div>').addClass(FILEUPLOADER_FILES_CONTAINER_CLASS)
      // @ts-expect-error
  ...
  _renderFile(file) {
    ...
    const $fileContainer = $('<div>').addClass(FILEUPLOADER_FILE_CONTAINER_CLASS).appendTo(this._$filesContainer);
  ...
  _updateFileNameMaxWidth() {
    ...
    const filesContainerWidth = getWidth(this._$filesContainer.find(`.${FILEUPLOADER_FILE_CONTAINER_CLASS}`).first()) || getWidth(this._$filesContainer);
  ...
  _renderSelectButton() {
    const $button = $('<div>').addClass(FILEUPLOADER_BUTTON_CLASS).appendTo(this._$inputWrapper);
  ...
  _attachSelectFileDialogHandlers(target) {
    ...
    eventsEngine.on($target, addNamespace(keyUpEventName, DIALOG_TRIGGER_EVENT_NAMESPACE), e => {
  ...
  _renderUploadButton() {
    ...
    const $uploadButton = $('<div>').addClass(FILEUPLOADER_BUTTON_CLASS).addClass(FILEUPLOADER_UPLOAD_BUTTON_CLASS)
    // @ts-expect-error
    .appendTo(this._$content);
  ...
  _renderInputWrapper() {
    this._$inputWrapper = $('<div>').addClass(FILEUPLOADER_INPUT_WRAPPER_CLASS)
    // @ts-expect-error
    .appendTo(this._$content);
  }
  ...
  _renderWrapper() {
    const $wrapper = $('<div>').addClass(FILEUPLOADER_WRAPPER_CLASS).appendTo(this.$element());
    const $container = $('<div>').addClass(FILEUPLOADER_CONTAINER_CLASS).appendTo($wrapper);
