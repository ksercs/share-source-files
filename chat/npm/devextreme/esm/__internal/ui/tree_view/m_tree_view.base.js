...
class TreeViewBase extends HierarchicalCollectionWidget {
    ...
    switch (value) {
      ...
      case 'normal':
        if (selectAllExists) {
          ...
          (_this$_$selectAllItem = this._$selectAllItem) === null || _this$_$selectAllItem === void 0 || _this$_$selectAllItem.remove();
  ...
  _normalizeIconState($node, hasNewItems) {
    ...
    if ($loadIndicator.length) {
      ...
      (_LoadIndicator$getIns = LoadIndicator.getInstance($loadIndicator)) === null || _LoadIndicator$getIns === void 0 || _LoadIndicator$getIns.option('visible', false);
    }
    const $treeViewItem = this._getItem($node);
