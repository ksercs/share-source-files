...
import { getIntersection, removeDuplicates } from '../../core/utils/array';
...
import { allowScroll } from '../ui/text_box/m_utils.scroll';
function xor(a, b) {
  return (a || b) && !(a && b);
}
...
class TagBox extends SelectBox {
  ...
  _renderMultiTag($input) {
    ...
    const $tag = $('<div>').attr('id', tagId).addClass(TAGBOX_TAG_CLASS).addClass(TAGBOX_MULTI_TAG_CLASS);
    const args = {
  ...
  _shouldUpdateSelectedItems() {
    ...
    if (isDefined(selectedItems) && selectedItems.length !== ((_this$_selectedItems = this._selectedItems) === null || _this$_selectedItems === void 0 ? void 0 : _this$_selectedItems.length)) {
      return true;
    }
  ...
  _createTag(value, $input, tagId, displayValue) {
    return $('<div>').attr('id', tagId).addClass(TAGBOX_TAG_CLASS).data(TAGBOX_TAG_DATA_KEY, value).data(TAGBOX_TAG_DISPLAY_VALUE, displayValue).insertBefore($input);
  }
