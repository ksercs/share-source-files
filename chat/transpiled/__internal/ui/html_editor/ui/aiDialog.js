...
class AIDialog extends _m_baseDialog.default {
  ...
  _renderResultTextArea($container) {
    const $textArea = (0, _renderer.default)('<div>').appendTo($container);
    this._resultTextArea = new _text_area.default($textArea.get(0), {
  ...
  _getCopyButtonItem(config) {
    return _extends({
      ...
      widget: 'dxButton',
      options: {
        text: _message.default.format('dxHtmlEditor-aiCopy'),
        onClick: async () => {
          var _navigator;
  ...
  _getTryAgainButtonItem() {
    return {
      ...
      widget: 'dxButton',
      options: {
        text: _message.default.format('dxHtmlEditor-aiTryAgain'),
